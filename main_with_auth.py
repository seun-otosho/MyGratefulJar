    ...
    return (
        ...
        Div(cls="container-fluid mt-3")(
            Div(cls="row")(
                # Admin sidebar
                Div(cls="col-md-3")(
                    admin_dashboard_sidebar(auth_ctx)
                ),
                # Main content
                Div(cls="col-md-9")(
                    H1("Dashboard"),
                    P(f"Welcome to the admin area, {auth_ctx.session.display_name}!"),
                    
                    # Quick stats
                    admin_stats_cards(auth_ctx),
                    
                    # Recent activity
                    admin_recent_activity(auth_ctx)
                )
            )
        )
    )

def admin_dashboard_sidebar(auth_ctx: AuthContext):
    """Admin dashboard sidebar"""
    sidebar_items = get_admin_sidebar(auth_ctx)
    ...
    return Div(cls="card")(
        ...
        Div(cls="card-body p-0")(
            *[Div(cls="mb-3 p-3 border-bottom")(
                H6(section["section"]),
                Ul(cls="list-unstyled mb-0")(
                    *[Li(cls="mb-1")(
                        A(href=item["url"], cls="text-decoration-none d-block p-2 rounded hover-bg-light")(
                            I(cls=f"fa {item['icon']} mr-2"),
                            item["name"]
                        )
                    ) for item in section["items"]]
...
def admin_stats_cards(auth_ctx: AuthContext):
    """Admin statistics cards"""
    return Div(cls="row mb-4")(
        Div(cls="col-md-3")(
            Div(cls="card text-white bg-primary")(
                Div(cls="card-body")(
                    H5(cls="card-title")("Total Posts"),
                    H2("0"),  # TODO: Get from database
                    P(cls="card-text")("Published articles")
                )
            )
        ),
        Div(cls="col-md-3")(
            Div(cls="card text-white bg-success")(
                Div(cls="card-body")(
                    H5(cls="card-title")("Comments"),
                    H2("0"),  # TODO: Get from database
                    P(cls="card-text")("User comments")
                )
            )
        ),
        Div(cls="col-md-3")(
            Div(cls="card text-white bg-info")(
                Div(cls="card-body")(
                    H5(cls="card-title")("Users"),
                    H2("0"),  # TODO: Get from database
                    P(cls="card-text")("Registered users")
                )
            )
        ) if auth_ctx.is_admin() else Div(),
        Div(cls="col-md-3")(
            Div(cls="card text-white bg-warning")(
                Div(cls="card-body")(
                    H5(cls="card-title")("Views"),
                    H2("0"),  # TODO: Get from database
                    P(cls="card-text")("Total page views")
                )
            )
        )
    )

def admin_recent_activity(auth_ctx: AuthContext):
    ...
    return Div(cls="card")(
        ...
        Div(cls="card-body")(
            P("Recent activity will be displayed here."),
